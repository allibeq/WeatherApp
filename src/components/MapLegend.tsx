...
function MapLegend({mapType}: Props) {
  ...
  return (
      <div className="absolute top-4 right-4 z-10001 w-96 rounded-xl shadow-lg p-4 bg-background/60 border border-accent/70 flex flex-col gap-3">
        <h3 className="text-sm font-semibold text-foreground">{data.title}</h3>
          ...
          <div className="w-full flex justify-between text-xs text-foreground">
              <p>{data.stops[0].value} {data.unit}</p>
              <p>{data.stops[data.stops.length -1].value} {data.unit}</p>
          </div>
      </div>
  );
}
...
const mapTypeData: Record<string, LayerConfig> = {
    precipitation_new: {
        title: "Rain (mm)",
        unit: "mm",
        stops: [
